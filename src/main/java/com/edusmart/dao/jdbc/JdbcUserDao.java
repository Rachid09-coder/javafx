...
public class JdbcUserDao implements UserDao {
    ...
    public JdbcUserDao() {
        ensureUserFeatureColumnsExist();
    }

    private synchronized void ensureUserFeatureColumnsExist() {
        if (columnChecked) return;
        ensureColumnExists("email_assoc", "VARCHAR(255) DEFAULT NULL");
        ensureColumnExists("google_id", "VARCHAR(255) DEFAULT NULL");
        ensureColumnExists("face_descriptor", "TEXT DEFAULT NULL");
        ensureColumnExists("signature_path", "VARCHAR(500) DEFAULT NULL");
        columnChecked = true;
    }

    private void ensureColumnExists(String columnName, String definition) {
        String sqlCheck = "SHOW COLUMNS FROM " + TABLE + " LIKE ?";
        try (Connection connection = DbConnection.getConnection();
             PreparedStatement ps = connection.prepareStatement(sqlCheck)) {
            ps.setString(1, columnName);
            ResultSet rs = ps.executeQuery();
            if (!rs.next()) {
                String sqlAdd = "ALTER TABLE " + TABLE + " ADD COLUMN " + columnName + " " + definition;
                try (PreparedStatement psAdd = connection.prepareStatement(sqlAdd)) {
                    psAdd.executeUpdate();
                }
            }
        } catch (SQLException ex) {
            System.err.println("Warning: Could not verify/add " + columnName + " column: " + ex.getMessage());
        }
    }
    ...
    public Optional<User> findByEmail(String email) {
        String sql = "SELECT * FROM " + TABLE + " WHERE LOWER(TRIM(email)) = LOWER(TRIM(?))";
    ...
    public Optional<User> findByEmailAssoc(String emailAssoc) {
        String sql = "SELECT * FROM " + TABLE + " WHERE LOWER(TRIM(email_assoc)) = LOWER(TRIM(?))";
    ...
    public Optional<User> findByGoogleId(String googleId) {
        String sql = "SELECT * FROM " + TABLE + " WHERE google_id = ?";
        ...
        try (Connection connection = DbConnection.getConnection();
            ...
            throw new RuntimeException("Failed to fetch user by Google id", ex);
        }
    }

    @Override
    public Optional<User> findByFaceDescriptor(String faceDescriptor) {
        String sql = "SELECT * FROM " + TABLE + " WHERE face_descriptor = ?";
        ...
        try (Connection connection = DbConnection.getConnection();
            ...
            throw new RuntimeException("Failed to fetch user by face descriptor", ex);
        }
    }
    ...
    private void fillMutableColumns(PreparedStatement ps, User u) throws SQLException {
        ...
        ps.setString(5, PasswordUtil.hashIfNeeded(u.getPassword()));
