...
public class JdbcCourseDao implements CourseDao {
    ...
    public boolean create(Course course) {
        try (Connection connection = DbConnection.getConnection()) {
            JdbcCourseTableSchema.ensureCourseColumns(connection);
    ...
    public boolean delete(int id) {
        ...
        try (Connection connection = DbConnection.getConnection()) {
            JdbcCourseTableSchema.ensureCourseColumns(connection);
            deleteDependentsIfPresent(connection, id);
    ...
    private void deleteDependentsIfPresent(Connection connection, int courseId) throws SQLException {
        if (JdbcCourseTableSchema.tableExists(connection, "exam")
                && JdbcCourseTableSchema.columnExists(connection, "exam", "course_id")) {
            if (JdbcCourseTableSchema.tableExists(connection, "exam_submission")
                    && JdbcCourseTableSchema.columnExists(connection, "exam_submission", "exam_id")) {
                executeDelete(connection,
                        "DELETE FROM exam_submission WHERE exam_id IN (SELECT id FROM exam WHERE course_id = ?)",
                        courseId);
            }
            executeDelete(connection, "DELETE FROM exam WHERE course_id = ?", courseId);
        }

        if (JdbcCourseTableSchema.tableExists(connection, "course_progress")
                && JdbcCourseTableSchema.columnExists(connection, "course_progress", "course_id")) {
            executeDelete(connection, "DELETE FROM course_progress WHERE course_id = ?", courseId);
        }

        if (JdbcCourseTableSchema.tableExists(connection, "grade")
                && JdbcCourseTableSchema.columnExists(connection, "grade", "course_id")) {
            executeDelete(connection, "DELETE FROM grade WHERE course_id = ?", courseId);
        }
    }

    private void executeDelete(Connection connection, String sql, int courseId) throws SQLException {
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
