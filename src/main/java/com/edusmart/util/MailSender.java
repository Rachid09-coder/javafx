...
public class MailSender {
    ...
    /** Lightweight confirmation hook used by the shopping checkout flow. */
    public static boolean sendOrderConfirmation(String recipientEmail, String subject, String body) {
        if (recipientEmail == null || recipientEmail.isBlank()) {
            return false;
        }
        System.out.println("Email confirmation simulated for: " + recipientEmail + " | " + subject);
        return true;
    }
    ...
    /**
     * Builds a professional HTML email body for an Exam Grading result.
     */
    public static String buildExamGradingEmailBody(String studentName, String examTitle, String score, String feedback) {
        String content = "<p style='font-size:16px;color:" + C_TEXT + ";margin:0 0 20px 0;'>"
                + "Bonjour <strong>" + esc(studentName) + "</strong>,</p>"
                + "<p style='color:" + C_MUTED + ";margin:0 0 24px 0;'>Votre examen "
                + "<strong style='color:" + C_BLUE + ";'>" + esc(examTitle) + "</strong> a été corrigé.</p>"
                + buildResultCard("Note obtenue", score, C_BLUE_LT)
                + "<div style='margin-top:20px;padding:16px;background:#F1F5F9;border-radius:8px;border-left:4px solid " + C_PURPLE + ";'>"
                +   "<p style='margin:0 0 8px 0;font-weight:700;color:" + C_TEXT + ";'>Feedback de l'enseignant :</p>"
                +   "<p style='margin:0;color:" + C_MUTED + ";font-style:italic;'>" + esc(feedback) + "</p>"
                + "</div>";
        return wrapTemplate("Résultat de votre examen", content, "📝 Résultat d'Examen");
    }
