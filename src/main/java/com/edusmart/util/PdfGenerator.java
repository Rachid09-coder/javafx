...
public class PdfGenerator {
    ...
    /** Generates a PDF invoice for the shopping checkout flow. */
    public static java.nio.file.Path generateInvoice(java.nio.file.Path outputFile,
                                                     java.util.Map<com.edusmart.model.Product, Integer> cartItems,
                                                     double subtotal,
                                                     double discount,
                                                     double total,
        ...
        try (FileOutputStream fos = new FileOutputStream(outputFile.toFile())) {
            ...
            drawPageBackground(writer, doc, false);
            addSectionTitle(doc, "Facture de commande");

            addInfoTable(doc, new String[][]{
                {"Client", nvl(customerEmail, "N/A")},
                {"Sous-total", String.format("%.2f €", subtotal)},
                {"Réduction", String.format("%.2f €", discount)},
            ...
            Font headerFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 11, EDU_WHITE);
            table.addCell(styledCell(new Phrase("Produit", headerFont), EDU_BLUE, Element.ALIGN_CENTER));
            table.addCell(styledCell(new Phrase("Quantité", headerFont), EDU_BLUE, Element.ALIGN_CENTER));
            table.addCell(styledCell(new Phrase("Sous-total", headerFont), EDU_BLUE, Element.ALIGN_CENTER));

            Font valueFont = FontFactory.getFont(FontFactory.HELVETICA, 11, EDU_TEXT_MUTED);
            for (java.util.Map.Entry<com.edusmart.model.Product, Integer> entry : cartItems.entrySet()) {
                com.edusmart.model.Product product = entry.getKey();
                Integer quantity = entry.getValue();
                table.addCell(styledCell(new Phrase(product.getName(), valueFont), EDU_WHITE, Element.ALIGN_LEFT));
                table.addCell(styledCell(new Phrase(String.valueOf(quantity), valueFont), EDU_WHITE, Element.ALIGN_CENTER));
                table.addCell(styledCell(new Phrase(String.format("%.2f €", product.getPrice() * quantity), valueFont), EDU_WHITE, Element.ALIGN_RIGHT));
            }
    ...
    /**
     * Generates a PDF report for an exam correction.
     */
    public static File generateExamGradingReport(com.edusmart.model.Exam exam, 
                                                 com.edusmart.model.ExamSubmission submission, 
                                                 User student) throws Exception {
        String fileName = "Correction_" + student.getLastName() + "_" + exam.getTitle().replaceAll("\\s+", "_") + ".pdf";
        File file = new File(PDF_DIR, fileName);
        ...
        try (FileOutputStream fos = new FileOutputStream(file)) {
            ...
            // Header
            Font titleFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 20, EDU_BLUE);
            Paragraph title = new Paragraph("RAPPORT DE CORRECTION IA", titleFont);
            title.setAlignment(Element.ALIGN_CENTER);
            doc.add(title);
            
            addSectionTitle(doc, "Détails de l'Examen");
            addInfoTable(doc, new String[][]{
                {"Examen", exam.getTitle()},
                {"Matière", exam.getModuleName()},
                {"Étudiant", student.getFullName()}
            });

            addSectionTitle(doc, "Réponse de l'Étudiant");
            Font contentFont = FontFactory.getFont(FontFactory.HELVETICA, 11, EDU_TEXT);
            Paragraph answer = new Paragraph(submission.getStudentAnswer(), contentFont);
            answer.setSpacingAfter(15);
            doc.add(answer);

            addSectionTitle(doc, "Évaluation et Feedback");
            addResultsTable(doc, new String[][]{
                {"Note Finale", submission.getScore() != null ? String.format("%.2f / 20", submission.getScore()) : "En attente"},
                {"Taux de Plagiat", submission.getPlagiarismScore() != null ? String.format("%d %%", (int)(submission.getPlagiarismScore()*100)) : "0 %"}
            });

            Paragraph feedbackTitle = new Paragraph("Commentaires de l'enseignant / IA :", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 12, EDU_BLUE));
            doc.add(feedbackTitle);
            Paragraph feedback = new Paragraph(nvl(submission.getTeacherFeedback(), nvl(submission.getAiFeedback(), "Aucun commentaire.")), contentFont);
            doc.add(feedback);
