...
public class CategoryManagementController implements Initializable {
    ...
    private void setupTable() {
        ...
        if (actionsColumn != null) {
            actionsColumn.setCellFactory(col -> 
                new ModernActionCell.Builder<Category>()
                    .onEdit(this::handleEditRow)
                    .onDelete(this::handleDeleteRow)
                    .editTooltip("Modifier la catégorie")
    ...
    private void handleEditRow(Category category) {
        Stage owner = (Stage) categoriesTable.getScene().getWindow();
        if (CategoryFormController.openDialog(owner, category)) {
            loadData();
            showMessage("Catégorie modifiée.", false);
        }
    }
    ...
    private void handleDeleteRow(Category category) {
        confirmAndDelete(category);
    }

    private void confirmAndDelete(Category sel) {
