...
public class ShopManagementController implements Initializable {
    ...
    private void setupTable() {
        ...
        if (categoryColumn != null) {
            categoryColumn.setCellValueFactory(cd -> {
                Integer cId = cd.getValue().getCategoryId();
                return new SimpleStringProperty(categoryNames.getOrDefault(cId, "Catégorie #" + cId));
            });
        }
        
        // Configuration de la colonne Actions avec ModernActionCell
        if (actionsColumn != null) {
            actionsColumn.setCellFactory(col -> 
                new ModernActionCell.Builder<Product>()
    ...
    private void handleEditRow(Product product) {
        Stage owner = (Stage) productsTable.getScene().getWindow();
        if (ProductFormController.openDialog(owner, product)) {
            loadData();
            showMessage("Produit modifié.", false);
        }
    }
    ...
    private void handleDeleteRow(Product product) {
        confirmAndDelete(product);
    }

    private void confirmAndDelete(Product sel) {
        Alert conf = new Alert(Alert.AlertType.CONFIRMATION, "Supprimer produit \"" + sel.getName() + "\" ?", ButtonType.YES, ButtonType.NO);
    ...
    @FXML private void handleShopAnalysisAI(ActionEvent e) { SceneManager.getInstance().navigateTo(SceneManager.Scene.TEACHER_SHOP_ANALYSIS_AI); }
