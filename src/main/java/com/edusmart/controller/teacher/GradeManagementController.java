...
public class GradeManagementController implements Initializable {
    ...
    private void setupTable() {
        ...
        if (actionsColumn != null) {
            actionsColumn.setCellFactory(col -> 
                new ModernActionCell.Builder<Grade>()
                    .onEdit(this::handleEditRow)
                    .onDelete(this::handleDeleteRow)
                    .editTooltip("Modifier la note")
                    .deleteTooltip("Supprimer la note")
                    .build()
            );
        }
    ...
    private void handleEditRow(Grade grade) {
        Stage owner = (Stage) gradesTable.getScene().getWindow();
        if (GradeFormController.openDialog(owner, grade)) {
            loadData();
            showMessage("Note mise à jour avec succès !", false);
        }
    }
    ...
    private void handleDeleteRow(Grade grade) {
        confirmAndDelete(grade);
    }

    private void confirmAndDelete(Grade selected) {
