...
public class SignUpController implements Initializable {
    ...
    private void handleRegister(ActionEvent event) {
        ...
        if (googleEmailField != null && !googleEmailField.getText().trim().isEmpty()) {
            String googleEmail = googleEmailField.getText().trim();
            if (!isValidEmail(googleEmail)) {
                showError("Adresse Google invalide.");
                return;
            }
            newUser.setGoogleId(toGoogleId(googleEmail));
        }
        if (faceDescriptorField != null && !faceDescriptorField.getText().trim().isEmpty()) {
            newUser.setFaceDescriptor(normalizeFaceDescriptor(faceDescriptorField.getText()));
        }
