...
import com.edusmart.service.OrderEmailService;
    ...
    private void handlePayStripe(ActionEvent e) {
        ...
        new Thread(() -> {
            try {
                ...
                String targetEmail = (user.getEmail() != null && !user.getEmail().isBlank()) ? user.getEmail() : "support@edusmart.local";
                
                // Envoyer l'email en arrière-plan
                new Thread(() -> {
                    try {
                        File invoicePdf = PdfGenerator.generateInvoicePdf(orderId, user.getFullName(), tot, trackingNumber);
                        OrderEmailService.sendOrderConfirmationEmail(targetEmail, user.getFirstName(), orderId, trackingNumber, invoicePdf);
